            ...
            {filteredProducts.length > 0 ? (
              filteredProducts.map(p => (
                <Link to={`/product/${p.id}`} key={p.id} className="group bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200 dark:border-white/10 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 overflow-hidden flex flex-col cursor-pointer">
                  <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-white dark:bg-slate-800 mb-6 flex-shrink-0 border border-slate-100 dark:border-white/5">
                    <img src={p.image} alt={p.name} className="absolute inset-0 w-full h-full object-contain p-6 transition-transform duration-700 group-hover:scale-105" />
                    {p.badge && (
